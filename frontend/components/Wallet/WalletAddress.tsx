...
export default function WalletAddress() {
  ...
  if (!ready) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 text-indigo-100/50">
      ...
      {wallets.length > 0 ? (
        wallets.map((wallet) => {
          ...
          return (
            ...
            >
              <Wallet size={16} className="text-indigo-100/70" />
              <span className="font-mono text-sm text-indigo-100/70">{short}</span>
              <button
              ...
              {isCopied && (
                <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-900 text-indigo-100 text-xs px-2 py-1 rounded shadow-lg z-10">
                  <span>Copied!</span>
        ...
        <div className="flex items-center gap-2 px-3 py-2 text-indigo-100/70 bg-indigo-500 rounded-lg">
          <Wallet size={16} />
